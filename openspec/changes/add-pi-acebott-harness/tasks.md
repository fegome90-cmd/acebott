...
- [x] 1.1 Create `harness/` directory structure (src/tools, src/lib,
      firmware, skill, tests/unit, tests/integration)
- [x] 1.2 Write `package.json` with pnpm, vitest, typebox deps
      (`@earendil-works/pi-coding-agent`, `@sinclair/typebox`,
      `@earendil-works/pi-ai`, `vitest`, `typescript`)
- [x] 1.3 Write `tsconfig.json` (strict, ESM, target ES2022)
- [x] 1.4 Write `vitest.config.ts` (unit dir default, integration opt-in
      via `ACEBOTT_HW` env)
- [x] 1.5 Write `.gitignore` (node_modules, dist, build artifacts)
- [x] 1.6 Run `pnpm install` and confirm deps resolve
- [x] 1.7 Run `pnpm build` (tsc --noEmit) and confirm 0 errors

## Phase 2: Constants and Libs

- [x] 2.1 Write `src/lib/constants.ts` (FQBN, BAUD, ESPTOOL_PATH,
      FLASH_OFFSETS, CHIP, PINS, AP, SERIAL_TIMEOUT_MS) per design
- [x] 2.2 Write `src/lib/usb.ts` — `detectChip()` using ioreg primary,
      system_profiler fallback (OQ-1)
- [x] 2.3 Write `src/lib/serial.ts` — `readSerial()` with poll-for-line
      and timeout (OQ-2)
- [x] 2.4 Write `src/lib/arduino-cli.ts` — `compileSketch()` with FQBN
      and build-path
- [x] 2.5 Write `src/lib/esptool.ts` — `flashSketch()` with hardcoded
      baud 115200 and offsets (ADR-005), signal support
- [x] 2.6 Add esptool path existence check in detect flow (RISK-04
      mitigation) — return clear error if path missing

## Phase 3: Firmware

- [x] 3.1 Write `firmware/health_check/health_check.ino` with:
...
- [x] 3.2 Verify firmware compiles with `arduino-cli compile --fqbn
      esp32:esp32:esp32`

## Phase 4: Tools

- [x] 4.1 Write `src/tools/detect.ts` — `robot_detect` tool:
...
- [x] 4.2 Write `src/tools/health.ts` — `robot_health` tool:
      - Call detect logic
      ...
      - Return report
- [x] 4.3 Write JSON-lines parser (inline in health.ts or separate
      `src/lib/parser.ts`) — skip malformed, detect terminal "health"
      line
- [x] 4.4 Write `src/index.ts` extension factory — register both tools
      via `pi.registerTool`, import types from
      `@earendil-works/pi-coding-agent`

## Phase 5: Skill

- [x] 5.1 Write `skill/SKILL.md` — domain skill:
